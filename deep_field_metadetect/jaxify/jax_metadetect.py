...
def _process_detections_for_shear(
    ...
    Parameters
    ----------
    mcal_obs : DFMdetObservation
        Metacalibrated observation for this shear
    x_coords : array
        Detection x coordinates (padded to max_objects)
    y_coords : array
        Detection y coordinates (padded to max_objects)
    det_flag : array
        Detection flags (1=real, 0=fill)
    bmask_flags : array
        Bmask flags at detection locations
    psf_res : dict
        PSF measurement result
    max_objects : int
        Maximum number of objects
    moment_stamp_size : int
        Stamp size for moments
    image_fft_size : int
        FFT size for images
    shear_idx : int
        Shear index
    ...
    -------
    batch_results : dict
        Detection results for this shear
    """

    # Compute mfrac values
    ...
    padded_mbobs = jax_get_mb_obs(mcal_obs, pad_width=pad_width)
...
def _jax_single_band_deep_field_metadetect_core(
    obs_wide,
    obs_deep,
    obs_deep_noise,
    nxy,
    ...
    """JIT-compiled core function for single-band observations using JAX peak finder.

    This is the jitable core implementation. Use `jax_single_band_deep_field_metadetect`
    for the wrapper with additional options.
    ...
    result : dict
        ...
        - "dfmdet_res" : dict (always present)
            ...
            - id, x, y, mdet_step_idx (int), bmask_flags, mfrac
            - wmom_flags, wmom_g1, wmom_g2, wmom_T_ratio, wmom_psf_T, wmom_s2n
            Note: mdet_step_idx is an integer. Wrapper function converts to strings.
    ...
    for shear_idx, shear in enumerate(shears):
        ...
        # Use shared jitted function for all post-detection processing
        # This includes: mfrac computation, sub-obs extraction, and moment measurements
...
def jax_single_band_deep_field_metadetect(
    ...
    # Use the jitted core function when not using SEP
    if not use_sep:
        result = _jax_single_band_deep_field_metadetect_core(
            obs_wide=obs_wide,
            obs_deep=obs_deep,
            obs_deep_noise=obs_deep_noise,
            nxy=nxy,
    ...
    # SEP detection path (legacy, not jitted)
    mcal_res = jax_metacal_wide_and_deep_psf_matched(
        obs_wide=obs_wide,
        obs_deep=obs_deep,
        obs_deep_noise=obs_deep_noise,
    ...
    for shear_idx, shear in enumerate(shears):
        ...
        # Use jitted function for all post-detection processing
        # This includes: mfrac computation, sub-obs extraction, and moment measurements
        batch_results = _process_detections_for_shear(
            mcal_res[shear],
            x_coords,
...
def jax_multi_band_deep_field_metadetect_jitted(
    ...
    # vmap over first 3 observation arguments, broadcast all others
    vmapped_metacal = jax.vmap(
        jax_metacal_wide_and_deep_psf_matched,
        in_axes=(
            0,  # obs_wide (vmapped)
            0,  # obs_deep (vmapped)
            0,  # obs_deep_noise (vmapped)
            None,  # nxy
            None,  # nxy_psf
            None,  # shears
            None,  # step
            None,  # skip_obs_wide_corrections
            None,  # skip_obs_deep_corrections
            None,  # return_noshear_deep
            None,  # return_k_info
            None,  # force_stepk_field
            None,  # force_maxk_field
            None,  # force_stepk_psf
            None,  # force_maxk_psf
            None,  # psf_fft_size
            None,  # image_fft_size
            None,  # reconv_psf_dk
            None,  # reconv_psf_kim_size
        ),
    )
